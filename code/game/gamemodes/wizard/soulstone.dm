...
/obj/structure/constructshell
	name = "wraithbone shell"
	icon = 'icons/obj/wizard.dmi'
	icon_state = "construct"
	desc = "An empty body crafted of wraithbone. It is inactive"
	...
	switch(choice)
		...
		if("VICTIM")
			...
			if(ticker.mode.name == "cult" && T.mind == ticker.mode:sacrifice_target)
				if(iscultist(U))
					U << "<span class='danger'>The Chaos Gods wants this mortal sacrificed with the rune.</span>"
				else
...
obj/item/proc/init_shade(var/obj/item/device/soulstone/C, var/mob/living/carbon/human/T, var/mob/U as mob, var/vic = 0)
	if(U != T)
		T << "Your soul has been captured! You are now bound to [U.name]'s will, help them suceed in their goals at all costs."
	else
		T << "You preserve your essence in your spirit stone."
	...
	if(iscultist(U))
		...
		S.mind.special_role = "shade"
	S.cancel_camera()
	C.icon_state = "soulstone2"
	C.name = "Soul Stone: [S.real_name]"
	C.imprinted = "[S.name]"
	if(vic)
		U << "<span class='info'><b>Capture successful!</b>:</span> [T.real_name]'s soul has been ripped from their body and stored within the spirit stone."
