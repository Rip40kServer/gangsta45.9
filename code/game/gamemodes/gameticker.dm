		...
		if(station_missed)
			...
			for(var/mob/M in mob_list)
				...
				if(M.stat != DEAD)
					...
					if(T && T.z==1)
						M.death(0) //no mercy
			for(var/turf/simulated/wall/WT in world) //Shows a little bit of the fire from a meltabomb while hopefully not being very performance intensive.
				if(WT.z == 1 && prob(20))
					spawn(pick(10, 20, 30, 40, 50, 60, 70, 80))
						WT.thermite += 60
						WT.thermitemelt()
		...
		if(captainless)
			for(var/mob/M in player_list)
				if(!istype(M,/mob/new_player))
					M << "A leadership role is not forced on anyone..... cowards."
		...
		if(!mode.explosion_in_progress && mode.check_finished())
			...
			spawn(50)
				if (mode.station_was_nuked)
					...
					if(!delay_end)
						world << "\blue <B>Rebooting due to destruction of the base in [restart_timeout/10] seconds</B>"
				else
