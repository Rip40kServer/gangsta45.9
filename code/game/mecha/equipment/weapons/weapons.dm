...
/obj/item/mecha_parts/mecha_equipment/weapon/energy/vanquisher
	equip_cooldown = 80
	name = "Vanquisher Cannon"
	desc = "A massive cannon that shoots explosive armor-piercing rounds. Found on a Vanquisher Pattern Leman Russ Tank."
	icon_state = "mecha_vanquisher"
	energy_drain = 600
	origin_tech = "materials=3;combat=8;powerstorage=4"
	projectile = /obj/item/projectile/bullet/vanquisher
...
/obj/item/mecha_parts/mecha_equipment/weapon/ballistic/autocannon					//Autocannon
	name = "Imperial Mounted Autocannon"
	desc = "The Imperial Mounted Autocannon fires very large antipersonel rounds. It's nothing personal."
	icon_state = "mecha_carbine"
	equip_cooldown = 10
	projectile = /obj/item/projectile/bullet/auto
	projectiles = 30000
	projectile_energy_cost = 1
	...
	action(atom/target)
		...
		var/target_y = targloc.y
		var/target_z = targloc.z
		var/turf/newlock = get_turf(chassis)
		targloc = null
		spawn	for(var/i=1 to min(projectiles, projectiles_per_shot))
			...
			if (targloc == curloc)
				continue
			var/obj/item/projectile/A = new projectile(curloc)
			playsound(src, 'sound/weapons/gunshot4.ogg', 100, 1)
			src.projectiles--
			A.firer = chassis.occupant
			A.original = target
			A.current = curloc
...
/obj/item/mecha_parts/mecha_equipment/weapon/ballistic/incendiary					//Incendiary
	name = "Incendiary cannon"
	desc = "This Inquisition model autocannon uses incendiary rounds for maximum effect."
	icon_state = "mecha_carbine"
	equip_cooldown = 10
...
/obj/item/mecha_parts/mecha_equipment/weapon/ballistic/lmg					//Dread Minigun
	name = "AutoGun"
	...
	action(atom/target)
		if(!action_checks(target)) return
		playsound(src, 'sound/weapons/minigun.ogg', 100, 1)
		var/turf/targloc = get_turf(target)
		var/target_x = targloc.x
		var/target_y = targloc.y
		var/target_z = targloc.z
...
/obj/item/mecha_parts/mecha_equipment/weapon/ballistic/shur				//Wraithlord Shurikan Catapult
	name = "Shuriken Catapult"
	desc = "Effective against infantry."
	icon_state = "shuriken_pistol"
	equip_cooldown = 10
	projectile = /obj/item/projectile/bullet/shurikan
	projectiles = 30000
	projectile_energy_cost = 2
	var/projectiles_per_shot = 7
	var/deviation = 0.4

	action(atom/target)
		if(!action_checks(target)) return
		playsound(src, 'sound/weapons/shur.ogg', 100, 1)
		var/turf/targloc = get_turf(target)
		var/target_x = targloc.x
		var/target_y = targloc.y
		var/target_z = targloc.z
		...
		spawn	for(var/i=1 to min(projectiles, projectiles_per_shot))
			...
			A.process()
			sleep(2)
		spawn	for(var/i=1 to min(projectiles, projectiles_per_shot))		//double the fun
			if(!chassis) break
			var/turf/curloc = get_turf(chassis)
			var/dx = round(gaussian(0,deviation),1)
			var/dy = round(gaussian(0,deviation),1)
		...
		log_message("Fired from [src.name], targeting [target].")
		do_after_cooldown()
		var/announce = pick('sound/mecha/wraithlordtrashtalk.ogg','sound/mecha/wraithlordtrashtalk2.ogg')
		if(prob(5))
		 playsound(loc, announce, 75, 0)
		return

/obj/item/mecha_parts/mecha_equipment/weapon/ballistic/sentlas				//sentinal laser
	name = "\improper Heavy Lasgun"
	desc = "A weapon for combat Sentinals. Definitely not an illegal stutter las..."
	icon_state = "mecha_uac2"
	equip_cooldown = 10
	projectile = /obj/item/projectile/bullet/lasgun2
	projectiles = 30000
	projectile_energy_cost = 20
	...
	action(atom/target)
		...
		spawn	for(var/i=1 to min(projectiles, projectiles_per_shot))
			...
			src.projectiles--
			A.firer = chassis.occupant
			playsound(src, 'sound/weapons/lasgun.ogg', 100, 1)
			A.original = target
			A.current = curloc
			A.yo = targloc.y - curloc.y
			A.xo = targloc.x - curloc.x
...
/obj/item/mecha_parts/mecha_equipment/weapon/ballistic/bettersentlas
	name = "\improper Multi-Las"
	desc = "A weapon mounted on imperial armor."
	icon_state = "mecha_uac2"
	equip_cooldown = 10
	projectile = /obj/item/projectile/bullet/lasgun3
	projectiles = 30000
	projectile_energy_cost = 20
...
/obj/item/missile
	...
	icon_state = "missile"
	var/primed = null
	var/obj/mecha/combat/firer = null
	throwforce = 15
...
/obj/item/mecha_parts/mecha_equipment/weapon/ballistic/brightlance			//brightlance
	name = "Brightlance"
	desc = "Effective against vehicles"
	icon_state = "mecha_ion"
	projectile = /obj/item/projectile/bullet/brightlance
	fire_sound = 'sound/weapons/brightlance.ogg'
	projectiles = 8000
	projectile_energy_cost = 2
	equip_cooldown = 30


/obj/item/projectile/bullet/brightlance
	icon = 'icons/obj/grenade.dmi'
	icon_state = "brightlance"
	var/life = 20

	Bump(var/atom/target, var/blocked = 0)
		if(istype(target,/obj/structure/)||istype(target,/obj/machinery/door/)||istype(target,/obj/mecha/combat))
			target.ex_act(2)
		..()
...
/obj/item/mecha_parts/mecha_equipment/weapon/ballistic/taurapid				//Tau Rapid Fire plasma
	name = "\improper Plasma Chaingun"
	desc = "A weapon for combat exosuits. Shoots a rapid chain of death."
	icon_state = "mecha_uac2"
	equip_cooldown = 10
	...
	action(atom/target)
		...
		set_ready_state(0)
		log_message("Fired from [src.name], targeting [target].")
		var/announce = pick('sound/mecha/rapidfire.ogg')
		if(prob(5))
		 playsound(loc, announce, 75, 0)
		do_after_cooldown()
		return
...
/*
Tau Flamer (god help us all)
*/

/obj/item/mecha_parts/mecha_equipment/weapon/ballistic/flamer				//Tau Flamer
	name = "\improper Antipersonel Flamer"
	desc = "It's nothing personel."
	icon_state = "mecha_uac2"
	equip_cooldown = 10
	...
	action(atom/target)
		...
		set_ready_state(0)
		log_message("Fired from [src.name], targeting [target].")
		var/announce = pick('sound/mecha/barragefire.ogg')
		if(prob(5))
		 playsound(loc, announce, 75, 0)
		do_after_cooldown()
		return

/obj/item/mecha_parts/mecha_equipment/weapon/ballistic/flamer2				//Tau Flamer
	name = "\improper Antipersonel Flamer"
	desc = "It's nothing personel."
	icon_state = "mecha_uac2"
	equip_cooldown = 10
	...
	action(atom/target)
		...
		set_ready_state(0)
		log_message("Fired from [src.name], targeting [target].")
		var/announce = pick('sound/mecha/stompem.ogg')
		if(prob(5))
		 playsound(loc, announce, 75, 0)
		do_after_cooldown()
		return

/obj/item/mecha_parts/mecha_equipment/weapon/ballistic/megacannon
	name = "\improper Mega Battle Cannon Array"
	desc = "Use a massive array of deadly cannons to shell the hell out of anything in range."
	icon_state = "mecha_vanquisher"
	equip_cooldown = 30
	...
	action(atom/target)
		if(!action_checks(target)) return
		for(var/obj/structure/banebladecannon/C in range(50, chassis))
			spawn(0)
				C.fire(target)
		set_ready_state(0)
		log_message("Fired from [src.name], targeting [target].")
...
/obj/item/mecha_parts/mecha_equipment/weapon/ballistic/banebolter
	name = "\improper Heavy Bolter Array"
	desc = "Use a massive array of deadly twin linked heavy bolters that will tear nearby opponents to pieces."
	icon_state = "mecha_vanquisher"
	equip_cooldown = 20
	projectile = /obj/item/projectile/bullet
	projectiles = 30000
	...
	action(atom/target)
		if(!action_checks(target)) return
		for(var/obj/structure/banebolter/BB in range(50, chassis))
			spawn(0)
				BB.fire(target)
		set_ready_state(0)
		log_message("Fired from [src.name], targeting [target].")
