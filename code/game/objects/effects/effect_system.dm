...
/datum/effect/effect/proc/fadeOut(var/atom/A, var/frames = 16) //Why in the emperor's name did you not just use the animate proc for this??? -Drake
...
/obj/effect/effect/chem_smoke/Move()
	...
	for(var/atom/A in view(1, src))
		if(reagents.has_reagent("radium")||reagents.has_reagent("uranium")||reagents.has_reagent("carbon")||reagents.has_reagent("thermite"))//Prevents unholy radium spam by reducing the number of 'greenglows' down to something reasonable -Sieve
			...
			if(creator && ismob(A))
				creator.reagent_exposure(A, reagents, 0) //Contact exposure caused by the smoke's maker and acting on an affected mob.
			reagents.reaction(A)
...
/datum/effect/effect/system/chem_smoke_spread
	...
	set_up(var/datum/reagents/carry = null, n = 5, c = 0, loca, direct, silent = 0)
		...
		if(!silent)
			...
			if(istype(carry.my_atom, /mob/living/carbon/alien/humanoid/tyranid/venomthropes))
				creator = carry.my_atom
			else
				if(carry.my_atom.fingerprintslast)
					var/mob/M = get_mob_by_key(carry.my_atom.fingerprintslast)
					if(M)
						creator = M
			log_game("A chemical smoke reaction has taken place in ([where])[contained].")
	...
	start()
		...
		for(i=0, i<src.number, i++)
			...
			spawn(0)
				...
				if(src.creator) smoke.creator = src.creator
				src.total_smoke++
				var/direction = src.direction
				if(!direction)
					if(src.cardinals)
				...
				for(i=0, i<pick(0,1,1,1,2,2,2,3,3,3,4,4), i++)
					sleep(10)
					step(smoke,direction)
				spawn(150+rand(10,30))
					if(smoke)
...
// except metal foams
/obj/effect/effect/foam/temperature_expose(datum/gas_mixture2/air, exposed_temperature, exposed_volume)
...
/datum/effect/effect/system/reagents_explosion
	...
	start()
		if (amount <= 2)
			...
			var/devastation = -1 //you spelled devestation wrong...
			var/heavy = -1
			var/light = -1
			var/flash = -1
			...
			if(source.fingerprintslast)
				var/mob/M = get_mob_by_key(source.fingerprintslast)
				if(M)
					creator = M
			explosion(location, devastation, heavy, light, flash, creator=creator)
