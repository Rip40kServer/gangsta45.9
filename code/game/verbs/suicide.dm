...
/mob/living/carbon/human/verb/suicide()
	...
	if(mind && mind.special_role == "Cultist")
		src << "\red You feel a mental block, stopping you from harming yourself in this manner. Instead, your consciousness is slipping away... In your current state, it is possible that an alternate personality will surface."
		message_admins("[src] ([src.key]) just tried to commit suicide as a chaos cultist. They will be mindswapped with a ghost if one is available.")
		src.ghostize(0) //Kicks them out of their body.
		request_ghost("A chaos cultist is without a player. Do you wish to be considered?", "Cultist Request", src)
		return

	if(mind && mind.special_role == "Genestealer Cult Member")
		src << "\red You feel a mental block, stopping you from harming yourself in this manner. Instead, your consciousness is slipping away... In your current state, it is possible that an alternate personality will surface."
		message_admins("[src] ([src.key]) just tried to commit suicide as a genestealer cultist. They will be mindswapped with a ghost if one is available.")
		src.ghostize(0)
		request_ghost("A genestealer cultist is without a player. Do you wish to be considered?", "Cultist Request", src)
		return
	...
	if(confirm == "Yes")
		if(!canmove || restrained())	//just while I finish up the new 'fun' suiciding verb. This is to prevent metagaming via suicide
			...
			return
		if(!suicide_allowed)
			src << "Suicide is not a part of the greater good."
			return
		suiciding = 1
