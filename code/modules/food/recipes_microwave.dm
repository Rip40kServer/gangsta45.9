...
/datum/recipe/combustion
	...
	result = /obj/item/weapon/reagent_containers/food/snacks/badrecipe
	time = 200 //Lets give it a bit of a longer fuse. Twice as much cook time.

	make_food(var/obj/container as obj)
		var/net_blast = 0
		for(var/obj/item/weapon/plasmisslea/M in container)
			qdel(M)
			if(usr)
				award(usr, "Do <b>NOT</b> try this at home...")
			net_blast += 4
			new /obj/effect/plasmacore(get_turf(container))
		for(var/obj/item/weapon/misslea/KM in container)
			if(usr) award(usr, "Cooking up trouble")
			qdel(KM)
			net_blast += 1
		for(var/obj/item/weapon/gun/energy/plasma/G in container)
			qdel(G)
			if(usr)
				award(usr, "The Darwin Award")
			net_blast += 3
			new /obj/effect/plasmacore(get_turf(container))
		for(var/obj/item/weapon/plastique/E in container)
			if(usr) award(usr, "Cooking up trouble")
			net_blast += 1
		for(var/obj/item/weapon/grenade/G in container)
			if(usr)
				if(istype(G, /obj/item/weapon/grenade/plasma))
					award(usr, "Do <b>NOT</b> try this at home...")
				else if(istype(G, /obj/item/weapon/grenade/chem_grenade/plague))
					award(usr, "<span class='nurgle'>Food Safety Permit Revoked</span>")
				else if(istype(G, /obj/item/weapon/grenade/vortex))
					award(usr, "\red Microwaved Chaos")
				else
					award(usr, "Cooking up trouble")
			if(istype(G, /obj/item/weapon/grenade/plasma))
				net_blast += 4
				new /obj/effect/plasmacore(get_turf(container))
				qdel(G)
			else if(istype(G, /obj/item/weapon/grenade/krak))
				net_blast += 1
				qdel(G)
			else if(istype(G, /obj/item/weapon/grenade/imperial))
				net_blast += 1
				qdel(G)
			else if(istype(G, /obj/item/weapon/grenade/stickbomb))
				net_blast += 1
				qdel(G)
			else if(istype(G, /obj/item/weapon/grenade/syndieminibomb))
				net_blast += 3
				qdel(G)
			else if(istype(G, /obj/item/weapon/grenade/iedcasing))
				if(prob(40)) net_blast += 1
				qdel(G)
			else
				spawn(2)
					G.loc = get_turf(src)
					G.prime()
		if(net_blast) //Lets a microwave that is completely filled with frags make one hell of a blast.
			net_blast = min(net_blast, 7) //Okay, we don't want anything bigger than this for an exploding microwave.
			explosion(get_turf(container), net_blast, net_blast + 2, net_blast + 3, net_blast + 4, shrapnel_count = 26)
			if(net_blast > 8) //Piercing explosions that will rip through more walls and hurt armor more, because this is pretending to be multiple bombs going off at once.
				spawn(6)
					explosion(get_turf(container), net_blast - 6, net_blast - 4, net_blast - 3, net_blast - 1, shrapnel_count = 6)
			if(net_blast > 6)
				spawn(4)
					explosion(get_turf(container), net_blast - 4, net_blast - 2, net_blast - 1, net_blast, shrapnel_count = 6)
			if(net_blast > 4)
				spawn(2)
					explosion(get_turf(container), net_blast - 2, net_blast, net_blast + 1, net_blast + 2, shrapnel_count = 6)
		var/obj/item/I = new /obj/item/weapon/reagent_containers/food/snacks/badrecipe(container)
		return I