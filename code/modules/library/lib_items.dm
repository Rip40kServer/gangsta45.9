...
/obj/item/weapon/chaosbook/attack_self(mob/user)
	if(carved)
		if(store)
			user << "<span class='notice'>[store] falls out of [title]!</span>"
			store.loc = get_turf(loc)
	...
	if(user.mind in ticker.mode.cult)
		...
		user << "\red You catch a glimpse of the Realm of Nurgle, The Plague Lord. You now see how flimsy the world is, you see that it should be open to the knowledge of Nurgle. A tome, a message from your new master, appears on the ground."
