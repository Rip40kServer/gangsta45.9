...
/obj/item/weapon/gun/proc/shoot_live_shot(mob/living/user as mob|obj, var/pointblank = 0, var/mob/pbtarget = null)
	...
	if(silenced)
		...
		src.luminosity = 3 //Make the gunfire flash a little bit. -Drake
		spawn(6) src.luminosity = 0 //And... Six tenths of a second later... Extinguish the light. NOTE: If we ever make attachable flash lights, a slightly fancier system will be needed to make everything work. Even now, spam clicking will make the light flutter a bit but this is not a serious concern because the lighting controller only processes these changes every tick, and the lighting controller's tick duration is only half a second.
