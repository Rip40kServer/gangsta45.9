	...
	if(!collateral)
		...
		if(istype(target,/mob/living))
			...
			if(istype(L,/mob/living/carbon/human))
				...
				if(!H) return
				user.act_of_violence(target, damage+10) //+10 because pretty much all beam weapons have additional effects that deal some extra damage through other means, be it ex_act, igniting the mob, or something else. I don't want to make effective_damage for beam weapons like normal projectiles get.
				...
				if(istype(user,/mob/living/carbon/human))
					...
					if(H.check_reflect("chest") && prob(80))
						return
				var/organ
				if(istype(user,/mob/living/carbon/human))
