...
/mob/dead/observer/verb/lookforgretchin()
	...
	set desc= "Look for a gretchin to become."
...
/mob/dead/observer/verb/lookfortyranid()
	...
	set desc= "Look for a tyranid larva to become."
...
/mob/dead/observer/proc/researchavailable()
	...
	if(ckey(src.key) in USED_RTD) //Lets see if this works to get single RTD enforced... -Drake	//only base krieger and avail allowed.
...
/*
	if(editchoice != "Cancel")
		USED_RTD.Add(ckey(src.key))		//This is not where the options are compiled. =P
*/
	switch(editchoice)
