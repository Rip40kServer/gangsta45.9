	...
	if (client.statpanel == "Status")
		if (internal)
			if (!internal.air_contents)
				...
				stat("Tank Pressure", internal.air_contents.get_pressure())
...
//				var/atom/target = get_edge_target_turf(user, get_dir(src, get_step_away(user, src)))
	...
	for(var/obj/item/organ/limb/temp in organs)
		switch(temp.name)
			if("head")
				update |= temp.take_damage(b_loss * 0.2, f_loss * 0.2, woundtype=/datum/wound/shrapnel)
			if("chest")
				update |= temp.take_damage(b_loss * 0.4, f_loss * 0.4, woundtype=/datum/wound/shrapnel)
			if("l_arm")
				update |= temp.take_damage(b_loss * 0.05, f_loss * 0.05, woundtype=/datum/wound/shrapnel)
			if("r_arm")
				update |= temp.take_damage(b_loss * 0.05, f_loss * 0.05, woundtype=/datum/wound/shrapnel)
			if("l_leg")
				update |= temp.take_damage(b_loss * 0.05, f_loss * 0.05, woundtype=/datum/wound/shrapnel)
