...
/mob/living/simple_animal/Life()
	...
	if(asleep)
		...
		return
	if(prob(2)) //Lets the simple mobs resume sleeping after a bit of time unobserved.
		var/observed = 0
...
/mob/living/simple_animal/attackby(var/obj/item/O as obj, var/mob/user as mob)  //Marker -Agouri
	if(istype(O, /obj/item/stack/medical))
		...
		if(O.force)
			user.act_of_violence(src, O.force)
			if(O.force >= force_threshold)
				var/damage = O.force
				if (O.damtype == STAMINA)
					damage = 0
				if(istype(O, /obj/item/weapon/nullrod) && istype(src, /mob/living/simple_animal/hostile/retaliate/daemon))
					adjustBruteLoss(20)
				if(istype(O, /obj/item/weapon/nullrod/conduit) && istype(src, /mob/living/simple_animal/hostile/retaliate/daemon))
					adjustBruteLoss(135)
				if(istype(O, /obj/item/weapon/twohanded/pchainsword) && istype(src, /mob/living/simple_animal/hostile/retaliate/daemon))
					adjustBruteLoss(25)
				adjustBruteLoss(damage)
