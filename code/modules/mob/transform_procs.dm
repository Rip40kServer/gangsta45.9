...
/mob/living/simple_animal/kbspawn/proc/kbize()
	var/mob/living/carbon/human/kb/O = new /mob/living/carbon/human/kb( src.loc )
	if(mind)		//TODO
		mind.transfer_to(O)
	else
		O.key = key
	...
	O.job = "syndicate"
	if(O.mind)
		O.mind.special_role = "Syndicate" //Set their allegiance to khorne.
	. = O
	qdel(src)
