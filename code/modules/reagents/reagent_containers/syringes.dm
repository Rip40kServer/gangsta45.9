		...
		switch(mode)
			...
			if(SYRINGE_INJECT)
				...
				if(ismob(target) && target != user)
					...
					user.reagent_exposure(M, reagents) //Log the reagent exposure with the automated system.
				...
				if(ismob(target) && target == user)
					...
					user.reagent_exposure(M, reagents)
	...
	update_icon()
		...
		if(reagents.total_volume)
			...
			switch(rounded_vol)
				...
				else if(10)	filling.icon_state = "syringe10"
				else if(15)	filling.icon_state = "syringe15"
				else filling.icon_state = "syringe15"
...
/obj/item/weapon/reagent_containers/syringe/viral
	name = "biohazardous syringe"
	desc = "You have a feeling the contents might be harmful."
	New()
		..()
		reagents.add_reagent("plague", 15)
		mode = SYRINGE_INJECT
		update_icon()
...
/obj/item/weapon/reagent_containers/syringe/toxic_treatment
	name = "Risanone Injector"
	desc = "Used to treat serious cases of poisoning. Risanone has soporific side effects but is also the only cure to a number of rarer blends of toxin."
	volume = 20
	amount_per_transfer_from_this = 10
	New()
		..()
		reagents.add_reagent("strongtox", 10)
		reagents.add_reagent("toxpurge", 10)
		mode = SYRINGE_INJECT
		update_icon()

/obj/item/weapon/reagent_containers/syringe/bruteburn_treatment
	name = "Kereline Injector"
	desc = "Used to treat serious cases of blunt trauma or burns. The side effects are numerous, but the healing speed is quite useful."
	volume = 25
	amount_per_transfer_from_this = 5
	New()
		..()
		reagents.add_reagent("brainburn", 5)
		reagents.add_reagent("toxbrute", 5)
		reagents.add_reagent("lho", 5)
		reagents.add_reagent("tricordrazine", 5)
		reagents.add_reagent("legecillin", 5)
		mode = SYRINGE_INJECT
		update_icon()

/obj/item/weapon/reagent_containers/syringe/epinephrine //The syringe in of itself is pretty effective, but mostly this contains an agent to make hyperadrenal stimulant.
	name = "syringe (emergency cardiac stabilization)"
	desc = "Contains a cocktail of drugs to stabilize an individual in critical condition."
	New()
		..()
