	...
	for(var/A in reagent_list)
		...
		if(M && R)
			if(M.reagent_check(R) != 1)
				if(M.stat == 2.0)
					if(!R.process_dead) //Only acts on reagents that specify that they work on dead people. Process dead is no longer working...?
						continue
				if(ishuman(M))
					...
					if(!H.heartbeating())
						if(!R.process_dead) //Only acts on reagents that specify that they work on dead people. Process dead is no longer working...?
							continue
				if(R.addictive && istype(M, /mob/living/carbon/human))
					var/mob/living/carbon/human/H = M
					var/addicted = 0
					for(var/datum/addiction in H.addictions)
						if(istype(addiction, R.addictiontype))
							addicted = 1
					if(!addicted && prob(R.addictive*2)) //Takes some time for the addiction to kick in. If you just set addictive to 1 it will be regularly addictive, 2 will be more so.
						var/datum/addiction/AD = new R.addictiontype
						AD.addictionid = R.id
						AD.addictionname = R.name
						H.addictions.Add(AD)
				R.on_mob_life(M)
...
datum/reagents/proc/net_effect(var/injected = 1, var/mob/living/M) //Evaluates the net effect of the chemical mixture. Positive means it is good for you, negative means it will hurt you.
	if(M.stat == DEAD) return 0 //Doesn't much matter what you inject into a corpse.
	var/total_effect = 0 //This is going to get pretty complicated if we are going to take the right things into effect. Still, it's worth it for a good automation system.
	if(injected)
		for(var/datum/reagent/toxin/T in reagent_list)
			total_effect -= (T.toxpwr*(T.volume/REAGENTS_METABOLISM)) //Standard toxin computation.
			if(istype(T, /datum/reagent/toxin/acid))
				total_effect -= (T.toxpwr*(T.volume/REAGENTS_METABOLISM)) //Count acid twice because it is a lot worse in your blood.
			if(istype(T, /datum/reagent/toxin/phenol))
				if(T.volume + M.reagents.get_reagent_amount(T.id) > 5)
					total_effect -= 25*T.volume
			if(istype(T, /datum/reagent/toxin/morphine) || istype(T, /datum/reagent/toxin/chloralhydrate))
				if(T.volume + M.reagents.get_reagent_amount(T.id) > 35) //Over 35, you are giving them a harmful amount.
					total_effect -= 5*T.volume //For each unit you are overdosing the, five units of harm are allocated. Below this the injections are harmless, but above this point you are straying close to lethal injections.
			if(istype(T, /datum/reagent/toxin/lexorin))
				total_effect -= (3*(T.volume/REAGENTS_METABOLISM))
			if(istype(T, /datum/reagent/toxin/slimejelly))
				total_effect -= (4*(T.volume/REAGENTS_METABOLISM))
	else
		for(var/datum/reagent/toxin/acid/A in reagent_list)
			total_effect -= 6*A.toxpwr
	return total_effect
