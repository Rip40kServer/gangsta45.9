...
/obj/item/weapon/coin/attack_self(mob/user as mob)
	if(cooldown < world.time - 15)
		...
		if(prob(abs(user.luck)/5)) //500 luck means perfect everything, including coin tossings. -500 luck means everything goes wrong for you, including coin tossings.
			if(user.luck > 0)
				coinflip = luckyside
			else
