...
/mob/living/carbon/human/OHinq/leader/proc/edit_faction_membership()
	var/editchoice
	editchoice = input("Select an action:","Faction Management") as null|anything in list("Add a Member","Remove a Member", "List members", "Add a Stormtrooper", "Remove a Stormtrooper", "List Stormtroopers", "Cancel")
	switch(editchoice)
		if("Add a Member")
			var/newname = strip_html_simple(input("Enter the ckey of the new member:","Add a Member","") as text, 30) //No ckeys will be >30 chars... Right?
			if(newname && trim(newname))
