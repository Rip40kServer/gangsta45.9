...
proc/anim(turf/location as turf,target as mob|obj,a_icon,a_icon_state as text,flick_anim as text,sleeptime = 0,direction as num, offset_x = 0, offset_y = 0)
//This proc throws up either an icon or an animation for a specified amount of time.
//The variables should be apparent enough.
	var/atom/movable/overlay/animation = new(location)
	animation.pixel_x = offset_x
	animation.pixel_y = offset_y
	if(direction)
		animation.dir = direction
	animation.icon = a_icon
	animation.layer = target:layer+1
			...
			for (var/turf/B in refined_trg)
				...
				if(C_src.x_pos == C_trg.x_pos && C_src.y_pos == C_trg.y_pos)
					...
					if(istype(X, /turf/simulated) && istype(T, /turf/simulated))
						...
						sim.oxygen = T.oxygen
						sim.co2 = T.co2
						sim.sleepgas = T.sleepgas
						sim.promethium = T.promethium
						sim.poison = T.poison
...
//						X.sd_SetOpacity(!X.opacity)			//TODO: rewrite this code so it's not messed by lighting ~Carn
	...
	//if(toupdate.len) //This isn't necessary... -Drake
	//	for(var/turf/simulated/T1 in toupdate)
	//		T1.CalculateAdjacentTurfs()
	//		air_master.add_to_active(T1,1)
...
/proc/is_sharp(obj/item/W as obj)		// For the record, WHAT THE HELL IS THIS METHOD OF DOING IT?
	return ( \
		istype(W, /obj/item/weapon/butterfly)					  || \
		istype(W, /obj/item/weapon/complexknife)					  || \
		istype(W, /obj/item/weapon/powersword)						  || \
		istype(W, /obj/item/weapon/chainsword)						  || \
		istype(W, /obj/item/weapon/complexsword)						  || \
...
/obj/proc/atmosanalyzer_scan(var/datum/gas_mixture2/air_contents, mob/user, var/obj/target = src)
	var/obj/icon = target
	user.visible_message("\red [user] has used the analyzer on \icon[icon] [target].</span>")
	var/pressure = air_contents.get_pressure()
	var/get_pressure = air_contents.get_pressure()

	user << "\blue Results of analysis of \icon[icon] [target]."
	if(get_pressure>0)
		var/o2_concentration = air_contents.oxygen/get_pressure
		var/co2_concentration = air_contents.co2/get_pressure
		var/plasma_concentration = air_contents.poison/get_pressure
		var/promethium_concentration = air_contents.promethium/get_pressure

		var/unknown_concentration =  1-(o2_concentration+co2_concentration+plasma_concentration+promethium_concentration)

		user << "\blue Pressure: [round(pressure,0.1)] kPa"
		user << "\blue Oxygen: [round(o2_concentration*100)]%"
		user << "\blue CO2: [round(co2_concentration*100)]%"
		user << "\blue Chlorine: [round(plasma_concentration*100)]%"
		user << "\blue Promethium: [round(promethium_concentration*100)]%"
...
/proc/center_image(var/image/I, x_dimension = 0, y_dimension = 0) //Grabbed from generic tgstation to make wings work.
	if(!I)
		return
